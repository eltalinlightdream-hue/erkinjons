...
import { Headphones, ExternalLink, Lock, Crown } from "lucide-react";
...
const TESTS: ListeningTest[] = [
  {
    ...
    title: "Dormancy",
    section: 4,
    description: "A lecture on animal dormancy — insects, the African lungfish, snails, and the Arctic ground squirrel.",
    questions: 10,
    htmlFile: "/passages/Dormancy_Section_4.html",
    audioFile: "/audio/part_4_dormancy.mp3",
    isPremium: false,
  },
];
...
function Listening() {
  ...
  const visible = filter === "all" ? TESTS : TESTS.filter(t => String(t.section) === filter);

  return (
    <SiteLayout>
      <section className="container mx-auto px-4 py-12 max-w-6xl">
        <h1 className="text-4xl md:text-5xl font-bold mb-3">IELTS Listening</h1>
        <p className="text-muted-foreground mb-2">Full listening tests with built-in audio player, timer, and answer checker.</p>
        ...
        {visible.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {visible.map((t) => {
              ...
              return (
                <Card key={t.id} className="p-6 flex flex-col relative overflow-hidden">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {!locked && <ExternalLink className="w-4 h-4 text-muted-foreground" />}
                    </div>
                  </div>
